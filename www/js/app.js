...
function show(feedObj, forceActive) {
    ...
    var obj = feedObj.story ? feedObj.story : feedObj.item
      , rtl = /[\u0600-\u06FF\u0750-\u077F]/.test(feedObj.title) || feedObj.title.toLowerCase().indexOf('arabic') > -1
        , ru = feedObj.title.toLowerCase().indexOf('моско') > -1 || feedObj.title.toLowerCase().indexOf('russian') > -1
      , fs = config.fs.toURL()
      , path = fs + (fs.substr(-1) === '/' ? '' : '/')
      , pullTop = $('<div/>', {
        id: 'pullrefresh-icon'
      ...
      , topBar = $('<div/>', {
        addClass: 'top-bar'
        , text: (ru ? 'Обновлено: ' : 'Updated: ') + (feedObj.friendlyPubDate !== undefined ? feedObj.friendlyPubDate : feedObj.lastBuildDate)
      })
      , ul = $('<ul/>', {})
      , container = $('<div/>', {
